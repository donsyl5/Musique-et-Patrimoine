            ...
            { 
              ...
              {         
                  ...
                  if($filename){
						...
						$titre_image = "cloche";
						rename("../assets/cloche/".$_FILES["fichier"]["name"], "../assets/cloche/".$titre_image.'.'.$imageFileType);
						$image = $titre_image.'.'.$imageFileType;
						  //echo "The file ". basename( $_FILES["fichier"]["name"]). " has been uploaded.";
						  $sql = "UPDATE cloche SET texte=:texte, image=:image, date_creation=:date_creation";
                        ...
                        if($result['image']!=$image){
                          chmod('../assets/cloche/', 0755);
                          @unlink("../assets/cloche/".$result['image']);
                        }
    							} else {
						  ...
						  $sql = "UPDATE cloche SET texte=:texte, date_creation=:date_creation";
						  $req1 = $bdd->prepare($sql);
						  $req1->execute(array(
							'texte' => $texte,
							'date_creation' => $date
		...
		?>
          ...
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">Les cloches de l'Eglise</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
			...
			<form role="form" method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" enctype="multipart/form-data">
              <div class="box-body">
                <div class="form-group">
                  <label>Texte Les cloches de l'Eglise</label>
                ...
                <div class="form-group">
					<label for="exampleInputFile">Image cloches de l'Eglise</label>
					<?php 
                      if(empty($_FILES["fichier"]["name"])){ echo '<img src="../assets/cloche/'.$result['image'].'" width="200"/>';} else { echo '<img src="../assets/cloche/'.$image.'.'.$imageFileType.'" width="10" />';} 
                    ?>
