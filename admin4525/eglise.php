            ...
            { 
              ...
              {         
                  ...
                  if($filename){
						...
						$titre_image = "eglise";
						rename("../assets/eglise/".$_FILES["fichier"]["name"], "../assets/eglise/".$titre_image.'.'.$imageFileType);
						$image = $titre_image.'.'.$imageFileType;
						  //echo "The file ". basename( $_FILES["fichier"]["name"]). " has been uploaded.";
						  $sql = "UPDATE eglise SET texte=:texte, image=:image, date_creation=:date_creation";
                        ...
                        if($result['image']!=$image){
                          chmod('../assets/eglise/', 0755);
                          @unlink("../assets/eglise/".$result['image']);
                        }
    							} else {
						  ...
						  $sql = "UPDATE eglise SET texte=:texte, date_creation=:date_creation";
						  $req1 = $bdd->prepare($sql);
						  $req1->execute(array(
							'texte' => $texte,
							'date_creation' => $date
		...
		?>
          ...
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">L'Eglise de Champcueil</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
			...
			<form role="form" method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" enctype="multipart/form-data">
              <div class="box-body">
                <div class="form-group">
                  <label>Texte Eglise de Champcueil</label>
                ...
                <div class="form-group">
					<label for="exampleInputFile">Image Eglise de Champcueil</label>
					<?php 
                      if(empty($_FILES["fichier"]["name"])){ echo '<img src="../assets/eglise/'.$result['image'].'" width="200"/>';} else { echo '<img src="../assets/eglise/'.$image.'.'.$imageFileType.'" width="10" />';} 
                    ?>
